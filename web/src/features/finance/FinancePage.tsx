...
import type { FinancialProjection } from '../../api/types'
...
function Projection({ projection }: { projection: FinancialProjection | null }) { if (!projection) return <section className="finance-guidance"><h2>Uma tarifa aprovada abre as projeções</h2><p>Revise a proposta de fonte oficial e aprove-a antes de registrar a primeira fatura.</p></section>; return <><section className="finance-hero"><div><p>Projeção estimada sem medidor</p><h1>{money(projection.totalMinor)}</h1><span>Sem solar: {money(projection.withoutSolarCompensationMinor)}</span></div><div><p>Saldo de créditos</p><strong>O saldo vem da última fatura</strong></div></section><section className="projection-breakdown"><h2>Real versus projetado</h2><p>A composição é estimada pelo servidor; o total informado na conta é a referência.</p><dl>{[['Consumo', projection.consumptionMinor], ['Compensação', projection.compensationMinor], ['Bandeira', projection.flagMinor], ['Tributos', projection.taxesMinor], ['CIP', projection.cipMinor]].map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{money(Number(value))}</dd></div>)}</dl></section></> }
...
export function FinancePage() {
  ...
  if (summary.isError || proposals.isError) return <section className="finance-state"><h1>Não foi possível carregar o financeiro.</h1><button className="secondary-action" onClick={() => { void refresh() }} type="button">Tentar novamente</button></section>
  return <article className="finance-page"><header><p className="eyebrow">Unidade geradora</p><h1>Financeiro solar</h1></header><Projection projection={summary.data?.latestProjection ?? null} />{proposal && <TariffProposalCard pending={false} proposal={proposal} onApprove={() => { void approveTariffProposal(proposal.id).then(refresh) }} />}{!proposal && !approved && <section className="finance-guidance"><h2>Sem tarifa aprovada</h2><p>Não há uma tarifa oficial disponível. Atualize a fonte da distribuidora antes de lançar a conta.</p></section>}<BillingCycleForm disabled={!approved} onSave={async (payload) => { await createBillingCycle(payload); await refresh() }} /></article>
}
