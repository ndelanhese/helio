...
export function TariffProposalCard({ proposal, onApprove, pending }: { proposal: TariffProposal; onApprove: () => void; pending: boolean }) {
	const local = proposal.sourceUrl === '/settings'
  return <section className="tariff-proposal" aria-labelledby="tariff-title">
    ...
    {local ? <a href="/settings">Editar tarifa em Configurações</a> : <a href={proposal.sourceUrl} rel="noreferrer" target="_blank">Abrir fonte oficial</a>}<small>Atualizada em {proposal.retrievedAt} · parser {proposal.parserVersion}</small>
