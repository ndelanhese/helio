...
func TestSetWeatherResultPublishesCurrentHourlyReadings(t *testing.T) {
	...
	runner.setWeatherResult(weather.Result{Available: true, Current: &weather.Current{At: at, TemperatureC: 22.4, PrecipitationMM: 0.3, WeatherCode: 61, CloudCoverPct: 78, WindSpeedKMH: 14.2, IrradianceWM2: 645.8}}, at)
	status := runner.WeatherStatus()
	if status.TemperatureC == nil || *status.TemperatureC != 22.4 || status.PrecipitationMM == nil || *status.PrecipitationMM != 0.3 || status.WeatherCode == nil || *status.WeatherCode != 61 || status.CloudCoverPct == nil || *status.CloudCoverPct != 78 || status.WindSpeedKMH == nil || *status.WindSpeedKMH != 14.2 || status.IrradianceWM2 == nil || *status.IrradianceWM2 != 645.8 {
